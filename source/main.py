...
def upload_to_github(local_path, remote_path):
    """
    Загружает локальный файл в репозиторий GitHub:
    - Если файл в репозитории отсутствует — создаёт новый.
    - Если существует и содержимое отличается — обновляет.
    - Если содержимое совпадает — пропускает операцию.
    Реализована простая логика повторных попыток при конфликте SHA.
    """
    # Проверяем, что локальный файл действительно существует
    ...
    # Читаем локальный файл в память для сравнения/загрузки
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            try:
                ...
                # Если были промежуточные конфликты — сообщим о них аккуратно
                if conflict_count:
                    log(f"⚠️ Были конфликты SHA при обновлении {remote_path} (повторов: {conflict_count}), но обновление выполнено.")
                else:
                ...
                if getattr(e_upd, "status", None) == 409 and attempt < max_retries:
                    ...
                    time.sleep(0.5)
                    # Не логируем промежуточные конфликты отдельно, чтобы не засорять вывод.
                    continue
                else:
                    msg = None
                    try:
