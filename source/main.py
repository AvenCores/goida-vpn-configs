...
def upload_to_github(local_path, remote_path):
    """
    Загружает локальный файл в репозиторий GitHub с улучшенной обработкой конфликтов.
    """
    ...
    for attempt in range(1, max_retries + 1):
        try:
            # Получаем актуальную информацию о файле при каждой попытке
            try:
            ...
            try:
                repo.update_file(
                    ...
                    content=content,
                    sha=current_sha,  # Используем актуальный SHA
                )
                ...
                if getattr(e_upd, "status", None) == 409:
                    if attempt < max_retries:
                        wait_time = 0.5 * (2 ** (attempt - 1))  # Экспоненциальная задержка
